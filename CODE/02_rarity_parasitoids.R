...
# per index x dataset combination.
# ------------------------------------------------------------
...
wilcox_results <- rarity_long %>%
  ...
  group_map(function(df, keys) {
    purrr::map_df(wilcox_pairs, function(pair) {
      x <- df %>% filter(guild_clean == pair[1]) %>% pull(dissimilarity)
      y <- df %>% filter(guild_clean == pair[2]) %>% pull(dissimilarity)
...
doc_wilcox <- officer::read_docx() %>%
  officer::body_add_par("Table S3b: Wilcoxon tests - rarity group comparisons",
                        style = "heading 1") %>%
