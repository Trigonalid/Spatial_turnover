...
# Fig. S11a — Bray-Curtis temporal dissimilarity (caterpillars, parasitoids)
# Fig. S11b — WN temporal dissimilarity (par-cat, cat-plant, cat-plant subsampled)
...
# ------------------------------------------------------------
# 3. FIGURE S11a — Bray-Curtis temporal dissimilarity
# ------------------------------------------------------------
Fig_S11a <- ggplot(bray_data_all, aes(x = x_group, y = value)) +
  geom_violin(
...
# ------------------------------------------------------------
# 4. FIGURE S11b — WN temporal dissimilarity
# ------------------------------------------------------------
Fig_S11b <- ggplot(plot_data_wn, aes(x = dataset, y = WN)) +
  geom_violin(
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
# ------------------------------------------------------------
# 5. Combined Figure S11
# ------------------------------------------------------------
Fig_S11 <- Fig_S11a + Fig_S11b + plot_layout(ncol = 2)

ggsave("output/fig/Figure_S11_temporal_Ohu.svg", Fig_S11,
       width = 25, height = 15, units = "cm", device = svglite)
ggsave("output/fig/Figure_S11_temporal_Ohu.pdf", Fig_S11,
       width = 25, height = 15, units = "cm")
ggsave("output/fig/Figure_S11_temporal_Ohu.png", Fig_S11,
